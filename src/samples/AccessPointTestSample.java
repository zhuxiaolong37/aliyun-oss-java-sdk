...
import com.aliyun.oss.internal.arn.Arn;
import com.aliyun.oss.internal.arn.OSSArnConverter;
import com.aliyun.oss.internal.arn.OSSResource;
...
public class AccessPointTest extends TestBase{
    ...
    private static String bucketName = "oss-java-sdk-test-example";
    ...
    @Ignore
    @Test
    public void testObjectRequestFromAp() throws Throwable {
        String arn = "arn:"+this.apArn;

        String md5 = BinaryUtil.toBase64String(BinaryUtil.calculateMd5(content.getBytes()));
        try {
            testCommonObjectCase(this.apClientTest, arn, this.ossClient, md5, true);
        } catch (OSSException e) {
            e.printStackTrace();
            Assert.fail("ErrorCode:" + e.getErrorCode() + "Message:" +e.getMessage());
        } finally {
            ...
            ObjectListing objectListing = this.apClientTest.listObjects(arn, "test-zxl");
            for (OSSObjectSummary objectSummary : objectListing.getObjectSummaries()) {
                existingKeys.add(objectSummary.getKey());
            }
            if(existingKeys.size() >0 ){
                DeleteObjectsRequest request = new DeleteObjectsRequest(arn);
                request.setKeys(existingKeys);
                this.apClientTest.deleteObjects(request);
            }
        }
    }
    ...
    private void testCommonObjectCase(OSS apClientTest, String apArn, OSS ossClient, String md5, boolean isArn) throws Throwable {
        ...
        // post object
        File postFile = createSampleFile(postObjectName, postObjectSize);
        String bucketName = apArn;
        String urlStr = null;
        if(isArn){
            OSSResource ossResource = OSSArnConverter.getInstance().convertArn(Arn.fromString(apArn));
            bucketName = OSSUtils.determineDomainPrefix(apArn, ossResource);
            urlStr = arnEndpoint.replace("http://", "http://" + bucketName+ ".");
        } else {
            urlStr = TestConfig.OSS_TEST_ENDPOINT.replace("http://", "http://" + bucketName+ ".");
        }


        Map<String, String> formFields = new LinkedHashMap<String, String>();
        formFields.put("key", postObjectName);
        formFields.put("Content-Disposition", "attachment;filename=" + postFile.getName());
        ...
        String signature = com.aliyun.oss.common.auth.ServiceSignature.create().computeSignature(TestConfig.OSS_TEST_ACCESS_KEY_SECRET, encodePolicy);
        formFields.put("Signature", signature);

        Assert.assertFalse(apClientTest.doesObjectExist(apArn, postObjectName));
        String ret = formUpload(urlStr, formFields, postFile);

        Assert.assertTrue(apClientTest.doesObjectExist(apArn, postObjectName));
        SimplifiedObjectMeta postObjectMeta = apClientTest.getSimplifiedObjectMeta(apArn, postObjectName);
        Assert.assertEquals(postObjectSize, postObjectMeta.getSize());
        Assert.assertTrue(apClientTest.doesObjectExist(apArn, postObjectName));
